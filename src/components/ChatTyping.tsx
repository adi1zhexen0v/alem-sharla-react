...
const ChatTyping = ({ correspondences }: ChatTypingProps) => {
  const listRef = useRef<HTMLDivElement>(null);
	...
	const correspondenceId = useAppSelector((state: RootState) => state.correspondence.id);
		...
		<div className='chat-typing'>
            ...
            {Object.entries(correspondences[correspondenceId])
              ...
              .map(([messageId, message]) => (
                <MessageItem
                  key={messageId}
                  message={message.imageURL ? message.imageURL! : message.fileURL ? message.fileURL! : message.text!}
                  isManager={message.isManager}
                  date={message.sentDate}
                  type={message.imageURL ? MessageTypes.IMAGE : message.fileURL ? MessageTypes.FILE : MessageTypes.TEXT}
                />
              ))}
          </div>
				</div>
